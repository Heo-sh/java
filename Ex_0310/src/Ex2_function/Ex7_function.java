...
//		메서드의 참조
//		- 람다식을 더욱 간결하게 표현
//		- 람다식이 하나의 메서드만 호출하는 경우에는 메서드 참조(method reference)라느
//		    방법으로 람다식을 간결하게 할 수 있다.
//		- 하나의 메서드만 호출하는 람다식은 
//		    클래스 이름::메서드 이름 or 
//		    참조변수::메서드 이름 로 바꿀 수 있다.
//		- 참조타입: 기본자료형을 제외하고 뱅열, 열거형, 클래스, 인터페이드 등
//		- 참조타입의 변수에는 객체의 주소가 저장된다.
		
		Function<String, Integer> f = (String s) -> Integer.parseInt(s);
		Function<String, Integer> f2 = Integer::parseInt; //메서드 참조
		
//		이미 생성된 객체의 메서드를 람다식에서 사용한 경우에는 클래스 이름 대신 그 객체의
//		이름을 적어주면 된다.
...
//		int[] nums = createArray(10, x -> new int[x]);
		int[] nums = createArray(10, int[]::new); //객체 생성
